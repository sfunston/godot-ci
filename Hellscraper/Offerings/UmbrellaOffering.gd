...
@onready var big_cooldown = 8
@onready var big_cooldown_max
@export var on_cooldown = true
...
func _ready():
	...
	big_cooldown_max = big_cooldown

func _process(delta):
	if big_cooldown <= 0:
		await get_tree().create_timer(big_cooldown).timeout
		on_cooldown = false
		big_cooldown = big_cooldown_max
	big_cooldown -= delta
	if !on_cooldown:
		await get_tree().create_timer(big_cooldown).timeout
		on_cooldown = true
